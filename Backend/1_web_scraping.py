...
def create_df_scraping(path_files_scraping, path_driver):
    ...
    linkspage = [
        tag.get("href") for tag in page.findAll(attrs={"class": "btn btn-success"})
    ]
    ...
    columns_names = [
        ...
        "Año",
        "Cantidad Sentencias",
        "Adiciona Complementa",
        "Fecha Ultima Sentencia",
    ]
...
def create_new_dowload(df_work_now, DBengine):
    df_work_old = pd.read_sql(
        "SELECT * from vw_certificado_radicacion_finalizado", DBengine.connect()
    )
    print("Datos Historicos: ", df_work_old.shape)
    new_dowload = df_work_now.merge(
        df_work_old.drop_duplicates(),
        on=["Radicación", "Certificado"],
        how="left",
        indicator=True,
    )
    ...
    new_dowload.drop(
        new_dowload[
            new_dowload["Certificado"] == "Pendiente por subir soporte documental"
        ].index,
        inplace=True,
    )
    if not new_dowload.empty:
        db_tosql = (
            new_dowload.reset_index()[["Certificado", "Clase", "Descargar", "Ciudad"]]
            .drop_duplicates()
            .copy()
        )
        db_tosql.columns = ["certificado", "clase", "descargar", "ciudad"]
        db_tosql.to_sql("mt_sentencia", engine, if_exists="append", index=False)
        db_tosql = new_dowload.reset_index()[
            [
                "Certificado",
                "Radicación",
                "Nombre del Predio",
    ...
    engine.execute(
        "insert into tt_log_transaccion (operacion, comentario) values ('Webscraping','Registros validados "
        + str(df_work_now.shape)
        + "')"
    )
    print("Registros Nuevos:", new_dowload.shape)
    engine.execute(
        "insert into tt_log_transaccion (operacion, comentario) values ('Webscraping','Registros Nuevos "
        + str(new_dowload.shape)
        + "')"
    )
...
engine.execute(
    "insert into tt_log_transaccion (operacion, comentario) values ('Webscraping','Inicia proceso')"
)
df_scrap = create_df_scraping(path_files_scraping_1, path_driver_1)
create_new_dowload(df_scrap, engine)
engine.execute(
    "insert into tt_log_transaccion (operacion, comentario) values ('Webscraping','Fin proceso')"
)
